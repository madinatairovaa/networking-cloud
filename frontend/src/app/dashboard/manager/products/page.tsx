...
export default function ManagerProductsPage() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      setProducts(res.data.data.content || []); setTotalPages(res.data.data.totalPages || 0);
    } catch (err: any) { setError(err.response?.data?.message || 'Failed to load'); } finally { setLoading(false); }
  }, [page, search]);

  useEffect(() => { fetchProducts(); categoryApi.getAll().then(r => setCategories(r.data.data || [])).catch(() => {}); }, [fetchProducts]);

  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault(); setFormError(''); setFormLoading(true);
    try {
      await productApi.create({ ...form, price: parseFloat(form.price), wholesalePrice: form.wholesalePrice ? parseFloat(form.wholesalePrice) : null, minOrderQuantity: parseInt(form.minOrderQuantity), categoryId: form.categoryId || null });
      setShowCreate(false); setForm({ name:'', description:'', sku:'', price:'', wholesalePrice:'', minOrderQuantity:'1', size:'', color:'', material:'', brand:'', categoryId:'' }); fetchProducts();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold">Products</h2><p className="text-muted-foreground">Manage products</p></div>
        {hasPermission('CREATE_PRODUCTS') && <Button onClick={() => setShowCreate(!showCreate)}>+ Add Product</Button>}
      </div>
      ...
      {showCreate && <Card className="border-primary/20"><CardHeader><CardTitle className="text-lg">New Product</CardTitle></CardHeader><CardContent>
        ...
        <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2"><Label>Category</Label><select value={form.categoryId} onChange={e => setForm({...form, categoryId:e.target.value})} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"><option value="">None</option>{categories.map((c:any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
      ...
      <Card><CardContent className="p-0">
        ...
        : products.length === 0 ? <div className="p-12 text-center text-muted-foreground">No products found</div>
        : <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b bg-muted/50">
          <th className="px-4 py-3 text-left font-medium">Product</th><th className="px-4 py-3 text-left font-medium">SKU</th><th className="px-4 py-3 text-left font-medium">Price</th><th className="px-4 py-3 text-left font-medium">Category</th><th className="px-4 py-3 text-left font-medium">Status</th>
        </tr></thead><tbody>{products.map(p => <tr key={p.id} className="border-b hover:bg-muted/30 transition-colors">
          <td className="px-4 py-3 font-medium">{p.name}</td><td className="px-4 py-3 font-mono text-xs text-muted-foreground">{p.sku}</td>
          <td className="px-4 py-3">${p.price?.toFixed(2)}</td><td className="px-4 py-3 text-muted-foreground">{p.categoryName||'-'}</td>
