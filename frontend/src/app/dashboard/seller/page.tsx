...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    dashboardApi.getSeller()
      ...
      .catch(err => setError(err.response?.data?.message || 'Failed to fetch seller statistics'))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <div className="grid grid-cols-1 md:grid-cols-3 gap-4">{Array.from({length: 3}).map((_, i) => (
  ...
  const cards = [
    { label: 'Products Created', value: stats?.productsCreated || 0, icon: '📦', color: 'from-blue-500 to-blue-600' },
    { label: 'Total Products', value: stats?.totalProducts || 0, icon: '🏪', color: 'from-green-500 to-green-600' },
    { label: 'Categories', value: stats?.totalCategories || 0, icon: '🏷️', color: 'from-purple-500 to-purple-600' },
  ];

  return (
    <div className="space-y-6 animate-fade-in">
      <div><h2 className="text-2xl font-bold">Seller Dashboard</h2><p className="text-muted-foreground">Product overview</p></div>
      ...
      {stats?.productsByStatus && (
        <Card><CardHeader><CardTitle>Products by Status</CardTitle></CardHeader>
